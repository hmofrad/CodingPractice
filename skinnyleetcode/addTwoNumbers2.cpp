...
public:
    ...
    ListNode* addTwoNumbers(ListNode* l1, ListNode* l2) {
        ...
        while(not s1.empty() or not s2.empty() or carry)  {
            //printf("%d\n", carry);
            int sum = carry;
            if(not s1.empty()) {sum += s1.back(); s1.pop_back();}
            if(not s2.empty()) {sum += s2.back(); s2.pop_back();}
            if(head) { ListNode* node = new ListNode(sum%10); node->next = head; head=node; }
            else {head = new ListNode(sum%10);}
            carry = sum/10;
        }
        //printf("%d\n", carry);
        return head;
