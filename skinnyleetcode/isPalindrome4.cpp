...
public:
    ...
    bool isPalindrome(string s) {
        ...
        int right = s.length()-1;
        while(left<=right) {
            char l = (s[left]>='A' and s[left]<='Z') ? s[left]-'A'+'a' : s[left];
            char r = (s[right]>='A' and s[right]<='Z') ? s[right]-'A'+'a' : s[right];
            if(not((l>='a' and l<='z') or (l>='0' and l<='9'))) { left++; continue; }
            if(not((r>='a' and r<='z') or (r>='0' and r<='9'))) { right--; continue; }
