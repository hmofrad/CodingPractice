...
class Solution {
public:
    Node* dfs(vector<vector<int>>& grid, int row, int col, int width) {
        printf("[%d %d %d]\n", row, col, width);
        int v = grid[row][col];
        bool is_leaf = true;
        for(int i = row; i < row+width; i++) {
            for(int j = col; j < col+width; j++) {
                if(grid[i][j]!=v) { is_leaf = false; break; }
            }
            if(not is_leaf) { break; }
        }
        
        Node* root = new Node(v, is_leaf);
        if(is_leaf) { return root; }
        
        
        root->topLeft     = dfs(grid, row,         col,         width/2);
        root->topRight    = dfs(grid, row,         col+width/2, width/2);
        root->bottomLeft  = dfs(grid, row+width/2, col,         width/2);
        root->bottomRight = dfs(grid, row+width/2, col+width/2, width/2);
    ...
    Node* construct(vector<vector<int>>& grid) {
        ...
        if(nrows == 0 or ncols == 0 or nrows != ncols) { return nullptr; }
        
        return dfs(grid, 0, 0, nrows);
    }
};
